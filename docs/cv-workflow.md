...
- `scripts/build_cv.py`: reads those data files and creates `files/Espinoza_CV.pdf`.
...
The homepage `Download CV` button points to `files/Espinoza_CV.pdf`.
...
It commits the regenerated `files/Espinoza_CV.pdf` back to the repository automatically.
